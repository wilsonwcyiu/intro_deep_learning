...
def relu_derivative(activation: np.array):
    return np.array([relu_derivative_single_element(xi) for xi in activation]);

def relu_derivative_single_element(self, xi):
    if xi > 0:
        return 1
    elif xi <= 0:
        return 0
...
def grdmse(cost: float, yhat_list: float, activation_record_list: list):
    ...
    for idx in range(0, len(yhat_list)):
        ...
        weight_vector_grd[6] += dcost_dpred * relu_derivative(yhat_list[idx])
        weight_vector_grd[7] += dcost_dpred * relu_derivative(yhat_list[idx])
        weight_vector_grd[8] += dcost_dpred * relu_derivative(yhat_list[idx])

        # hidden layer to input layer grd
        activation_record = activation_record_list[idx]
        weight_vector_grd[0] += dcost_dpred * relu_derivative(activation_record[7])
        weight_vector_grd[1] += dcost_dpred * relu_derivative(activation_record[8])
        weight_vector_grd[2] += dcost_dpred * relu_derivative(activation_record[7])
        weight_vector_grd[3] += dcost_dpred * relu_derivative(activation_record[8])
        weight_vector_grd[4] += dcost_dpred * relu_derivative(activation_record[7])
        weight_vector_grd[5] += dcost_dpred * relu_derivative(activation_record[8])
...
def xor_net(x1_list, x2_list, weight_vector: list):
    ...
    for idx in range(0, len(x1_list)):
        ...
        output_activation: float = relu(output_neural_3_z)
