...
def relu_derivative(activation: np.array):
    return np.array([relu(xi) for xi in activation])
...
if __name__ == '__main__':
    ...
    learning_rate_dict: dict = {"Relu": None}
    ...
    for name, lr in learning_rate_dict.items():
        lr = 0.05
        low: float = -1
        high: float = 1
        ...
        for epoch in range(50000):
            ...
            layer_1_activation: np.array = relu(layer_1_z)
            ...
            layer_2_activation_yhat: np.array = relu(layer_2_z);                        #print("a2_yhat_activation", a2)
            ...
            output_layer_dpred_dz: np.array = relu_derivative(layer_2_activation_yhat);               #print("dpred_dz.shape", dpred_dz.shape)
            ...
            hidden_layer_dpred_dz: np.array = relu_derivative(layer_1_activation);     #print("hl_dpred_dz.shape", hl_dpred_dz.shape)
    ...
    plot_id = 1
    plot_title = "MSE in Relu activation"
    x_label = "Iterations"
    y_label = "MSE with Relu activation"
    plt = plot_multi_list(plot_id, plot_title, x_label, y_label, mse_data_dict)
    plt.show()


    plot_id = 2
    plot_title = "Misclassification count with Relu activation"
    x_label = "Iterations"
