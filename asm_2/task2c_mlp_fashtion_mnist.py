...
from hyper_para_mlp_dict import hyper_para_mlp_dict
import numpy as np
...
if __name__ == '__main__':
    ...
    for idx in range(x_train.shape[0]):
        x_train_permutated = np.random.permutation(x_train[idx])
        x_train[idx] = x_train_permutated

    for idx in range(x_valid.shape[0]):
        x_valid_permutated = np.random.permutation(x_valid[idx])
        x_valid[idx] = x_valid_permutated

    for idx in range(x_test.shape[0]):
        x_test_permutated = np.random.permutation(x_test[idx])
        x_test[idx] = x_test_permutated
    ...
    execute_dict_id_list = [10288]#10576
    ...
    result_output_file = folder_dir + date_time_str + "_permutated_result_mlp_fashion_output.csv"
    result_backup_output_file = folder_dir + date_time_str + "_permutated_result_mlp_fashion_output_backup.csv"
